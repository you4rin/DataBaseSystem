...
single_thread_test()
{
	...
	/* Initiate database. */
	init_db(DATABASE_BUFFER_SIZE, 0, 0, "log.db", "logmsg.txt");
...
int main(int argc, char** argv) {
    ...
    */
	...
	/*
	init_db(10000, 0, 0, NULL, NULL);
    for(int i = 1;i <= 10; ++i){
		std::string path = "DATA" + std::to_string(i) + ".db";
		//printf("%d\n", i);
		open_table((char*)path.c_str());
	}
	*/
	/*
	log_type test_log;
	printf("begin: %d\n",sizeof(test_log._begin));
	printf("update: %d\n",sizeof(test_log._update));
	printf("commit: %d\n",sizeof(test_log._commit));
	printf("rollback: %d\n",sizeof(test_log._rollback));
	printf("compensate: %d\n",sizeof(test_log._compensate));
	*/
